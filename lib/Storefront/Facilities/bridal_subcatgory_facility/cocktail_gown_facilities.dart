...
class CocktailGownFacilities extends StatefulWidget {
  final Map<String, dynamic> attributes;
  const CocktailGownFacilities({super.key, required this.attributes});

  @override
  State<CocktailGownFacilities> createState() => CocktailGownFacilitiesState();
}

class CocktailGownFacilitiesState extends State<CocktailGownFacilities>
    with FacilitiesHelpersMixin<CocktailGownFacilities> {
  ...
  List<String> _necklineTypes     = [];
  String?      _sleeveTypes;
  List<String> _fabricOptions     = [];
  List<String> _embellishmentTypes = [];
  String?      _trainLength;
  ...
  String?      _accessoryStylingSupport;
  ...
  void _setFields(Map<String, dynamic> attrs) {
    final cm = asMap(attrs['cocktail_gown_master']);
    ...
    final pc = asMap(cm['product_catalog']);
    _gownTypes      = toList(pc['gown_types']);
    _collectionType = pc['collection_type'] as String?;
    _designStyle    = toList(pc['design_style']);
    _colorPalette   = toList(pc['color_palette']);

    final ci = asMap(cm['core_intelligence']);
    _silhouetteTypes    = toList(ci['silhouette_types']);
    _necklineTypes      = toList(ci['neckline_types']);
    _sleeveTypes        = ci['sleeve_types'] as String?;
    _fabricOptions      = toList(ci['fabric_options']);
    _embellishmentTypes = toList(ci['embellishment_types']);
    _trainLength        = ci['train_length'] as String?;
    _weightCategory     = ci['weight_category'] as String?;

    final fs = asMap(cm['fit_styling']);
    _sizeRange               = fs['size_range'] as String?;
    _bodyTypeStyling         = toList(fs['body_type_styling']);
    _fitType                 = fs['fit_type'] as String?;
    ...
    _accessoryStylingSupport = fs['accessory_styling_support'] as String?;

    final ou = asMap(cm['occasion_usage']);
    ...
    final pl = asMap(cm['pricing_logic']);
    ...
    final pd = asMap(cm['production_delivery']);
    ...
    final so = asMap(cm['scale_operations']);
    ...
    final tags = asMap(cm['ai_tags']);
    _gownTags      = toList(tags['gown_tags']);
  ...
  Map<String, dynamic> buildMaster(Map<String, dynamic> ex) {
    return {
      ...
      "core_intelligence": {
        ...
        "neckline_types":      _necklineTypes,
        "sleeve_types":        _sleeveTypes,
        "fabric_options":      _fabricOptions,
        "embellishment_types": _embellishmentTypes,
        "train_length":        _trainLength,
      ...
      "fit_styling": {
        ...
        "accessory_styling_support": _accessoryStylingSupport,
      },
      "occasion_usage": {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        const Text("Cocktail Gown Master Profile",
            style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
        const SizedBox(height: 4),
        Text("Structured cocktail gown attributes for storefront and AI FAQ matching.",
        ...
        if (_cs1) ...[
          ...
          buildDropdown("Vendor Type",
            ["Designer Boutique", "Multi-Designer Store", "Custom Gown Studio",
        ...
        sectionHeader("Section 2 — Product catalog", _cs2,
            () => setState(() => _cs2 = !_cs2)),
        if (_cs2) ...[
          const SizedBox(height: 16),
          buildMultiSelect("Gown Types",
            ["Cocktail Party Gown", "Reception Gown", "Engagement Gown",
             "After-Party Gown", "Pre-Wedding Shoot Gown"],
            _gownTypes,
            (o, c) => setState(() => c ? _gownTypes.add(o) : _gownTypes.remove(o))),
          ...
          buildMultiSelect("Design Style",
            ["Minimal Elegant", "Glam / Sequined", "Indo-Western",
             "Western Formal", "Red Carpet Style", "Contemporary Fusion"],
            _designStyle,
            (o, c) => setState(() => c ? _designStyle.add(o) : _designStyle.remove(o))),
          buildMultiSelect("Color Palette",
            ["Black", "Red", "Metallic (Gold/Silver)", "Pastels",
             "Jewel Tones", "Nude / Beige", "Custom"],
            _colorPalette,
        ...
        sectionHeader("Section 3 — Core intelligence", _cs3,
            () => setState(() => _cs3 = !_cs3)),
        if (_cs3) ...[
          const SizedBox(height: 16),
          buildMultiSelect("Silhouette Types",
            ["A-line", "Ball Gown", "Mermaid / Trumpet", "Sheath / Column",
             "Empire Waist", "High Slit"],
            _silhouetteTypes,
            (o, c) => setState(() => c ? _silhouetteTypes.add(o) : _silhouetteTypes.remove(o))),
          buildMultiSelect("Neckline Types",
            ["Sweetheart", "Off-Shoulder", "Strapless", "Halter",
             "V-Neck", "High Neck", "One-Shoulder"],
            _necklineTypes,
            (o, c) => setState(() => c ? _necklineTypes.add(o) : _necklineTypes.remove(o))),
          buildDropdown("Sleeve Types",
            ["Sleeveless", "Cap Sleeves", "Full Sleeves", "Detachable Sleeves"],
            _sleeveTypes, (v) => setState(() => _sleeveTypes = v)),
          buildMultiSelect("Fabric Options",
            ["Satin", "Silk", "Tulle", "Net", "Organza", "Velvet", "Crepe"],
            _fabricOptions,
            (o, c) => setState(() => c ? _fabricOptions.add(o) : _fabricOptions.remove(o))),
          buildMultiSelect("Embellishment Types",
            ["Sequins", "Beads", "Crystals", "Embroidery", "Feathers", "Minimal / Plain"],
            _embellishmentTypes,
            (o, c) => setState(() => c ? _embellishmentTypes.add(o) : _embellishmentTypes.remove(o))),
          buildDropdown("Train Length",
            ["No Train", "Sweep Train", "Chapel Train", "Custom"],
            _trainLength, (v) => setState(() => _trainLength = v)),
        ...
        sectionHeader("Section 4 — Fit & styling", _cs4,
            () => setState(() => _cs4 = !_cs4)),
        if (_cs4) ...[
          const SizedBox(height: 16),
          ...
          buildDropdown("Fit Type",
            ["Body-hugging", "Relaxed Fit", "Structured Fit"],
            _fitType, (v) => setState(() => _fitType = v)),
          ...
          buildYesNo("Accessory Styling Support", _accessoryStylingSupport,
              (v) => setState(() => _accessoryStylingSupport = v)),
        ],
        dividerLine(),

        // ── Section 5: Occasion & Usage ────────────────────────────────────
        sectionHeader("Section 5 — Occasion & usage", _cs5,
            () => setState(() => _cs5 = !_cs5)),
        if (_cs5) ...[
          const SizedBox(height: 16),
          buildMultiSelect("Primary Occasion Suitability",
            ["Cocktail Night", "Reception", "Engagement", "After Party"],
        ...
        sectionHeader("Section 7 — Production & delivery", _cs7,
            () => setState(() => _cs7 = !_cs7)),
        if (_cs7) ...[
          const SizedBox(height: 16),
          buildDropdown("Production Time",
            ["Ready Stock", "1–3 weeks", "3–6 weeks", "6–10 weeks"],
        ...
        sectionHeader("Section 8 — Scale & operations", _cs8,
            () => setState(() => _cs8 = !_cs8)),
        if (_cs8) ...[
          const SizedBox(height: 16),
        ...
        sectionHeader("Section 9 — Workflow & booking", _cs9,
            () => setState(() => _cs9 = !_cs9)),
        if (_cs9) ...[
          const SizedBox(height: 16),
          buildDropdown("Advance Booking Time",
            ["<1 week", "1–3 weeks", "1–2 months", "2–4 months"],
        ...
        sectionHeader("Section 10 — Portfolio tagging (AI layer)", _cs10,
            () => setState(() => _cs10 = !_cs10)),
        if (_cs10) ...[
          const SizedBox(height: 16),
          buildMultiSelect("Gown Tags",
            ["Luxury Gowns", "Budget Gowns", "Designer Couture", "Custom Gowns"],
            _gownTags,
            (o, c) => setState(() => c ? _gownTags.add(o) : _gownTags.remove(o))),
          buildMultiSelect("Style Tags",
            ["Glamorous", "Minimal", "Modern", "Red Carpet"],
            _styleTags,
            (o, c) => setState(() => c ? _styleTags.add(o) : _styleTags.remove(o))),
          buildMultiSelect("Bride Type Tags",
            ["Modern Bride", "Experimental Bride", "Elegant Bride"],
            _brideTypeTags,
